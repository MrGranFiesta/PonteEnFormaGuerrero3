...
class ManagerDualRepsVM(
    ...
    override fun getInitTime(isInverse: Boolean): Long = descanso

    override fun isAutoPlay(isInverse: Boolean): Boolean = true
    ...
    override fun changeStateByCtrlTimer() {
        ...
        when {
            ...
            StateSerie.isDescanso(_stateSerie2.value) -> {
                countdownVM2.setActive(StateSerie.isPlay(isReverseState2))
                this.setStateSerie2(isReverseState2)
            }
        }
    ...
    override fun isPausedTimer(
        ...
        return when {
            StateSerie.isDescanso(stateSerie2) && StateSerie.isDescanso(stateSerie1) -> {
                StateSerie.isPause(stateSerie2) && StateSerie.isPause(stateSerie1)
            }

            StateSerie.isDescanso(stateSerie2) -> StateSerie.isPause(stateSerie2)
            StateSerie.isDescanso(stateSerie1) -> StateSerie.isPause(stateSerie1)
            else -> false
        }
    }
    ...
    fun isActiveNoWait(state: StateSerie) = StateSerie.ACTIVO_PLAY == state
    fun isDescanso(state1: StateSerie, state2: StateSerie) =
        StateSerie.isDescanso(state1) || StateSerie.isDescanso(state2)
    ...
    override fun onPause() {
        ...
        if (stateSerie2.value == StateSerie.DESCANSO_PLAY) {
            countdownVM2.stopCountdown()
        } else if (stateSerie2.value == StateSerie.DESCANSO_PAUSE) {
    ...
    override fun onResume() {
        ...
        } else if (stateSerie2.value == StateSerie.DESCANSO_PLAY) {
            countdownVM2.playCountdown()
        }
    }
